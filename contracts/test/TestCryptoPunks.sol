...
contract TestCryptoPunks is ICryptoPunksMarket {
    ...
    constructor() {
        ...
        punkIndexToAddress[0] = msg.sender;
        punkIndexToAddress[1] = msg.sender;
        punkIndexToAddress[2] = msg.sender;
        punkIndexToAddress[3] = msg.sender;
        punkIndexToAddress[4] = msg.sender;
        punkIndexToAddress[5] = msg.sender;
        punkIndexToAddress[6] = msg.sender;
        punkIndexToAddress[7] = msg.sender;
        punkIndexToAddress[8] = msg.sender;
        punkIndexToAddress[9] = msg.sender;
        punkIndexToAddress[10] = msg.sender;
        punkIndexToAddress[11] = msg.sender;
        punkIndexToAddress[12] = msg.sender;
        punkIndexToAddress[13] = msg.sender;
        punkIndexToAddress[14] = msg.sender;
        punkIndexToAddress[15] = msg.sender;
        punkIndexToAddress[16] = msg.sender;
        punkIndexToAddress[17] = msg.sender;
        punkIndexToAddress[18] = msg.sender;
        punkIndexToAddress[19] = msg.sender;
    ...
    function setInitialOwner(address to, uint256 punkIndex) public override {
    ...
    function setInitialOwners(address[] calldata addresses, uint256[] calldata indices) public override {
        if (msg.sender != owner) revert();
    ...
    function allInitialOwnersAssigned() public override {
        if (msg.sender != owner) revert();
        allPunksAssigned = true;
    }

    function getPunk(uint256 punkIndex) public override {
        if (!allPunksAssigned) revert();
    ...
    function offerPunkForSale(uint256 punkIndex, uint256 minSalePriceInWei) public override {
        if (!allPunksAssigned) revert();
        if (punkIndexToAddress[punkIndex] != msg.sender) revert();
        if (punkIndex >= 5) revert();
    ...
    function enterBidForPunk(uint256 punkIndex) public payable override {
        if (punkIndex >= 5) revert();
        if (!allPunksAssigned) revert();
        if (punkIndexToAddress[punkIndex] == address(0x0)) revert();
        if (punkIndexToAddress[punkIndex] == msg.sender) revert();
    ...
    function acceptBidForPunk(uint256 punkIndex, uint256 minPrice) public override {
        if (punkIndex >= 5) revert();
        if (!allPunksAssigned) revert();
        if (punkIndexToAddress[punkIndex] != msg.sender) revert();
    ...
    function withdrawBidForPunk(uint256 punkIndex) public override {
        if (punkIndex >= 5) revert();
        if (!allPunksAssigned) revert();
        if (punkIndexToAddress[punkIndex] == address(0x0)) revert();
        if (punkIndexToAddress[punkIndex] == msg.sender) revert();
